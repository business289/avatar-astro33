...
// Lensflare removed — wrong texture files were rendering as Earth-map rectangles
...
scene.background = new THREE.Color(0x010101); // near-pure-black deep space
...
// Lighting — physically-based solar system illumination
// Eclipse tour restores intensity=0.5 so keep base at 0.5
const ambientLight = new THREE.AmbientLight(new THREE.Color(0.08, 0.07, 0.07), 0.5); // near-black warm, no blue bias
scene.add(ambientLight);

// Hemisphere light: nearly-black to prevent blue cast on planets
// Keep it tiny so the sun's point light provides all real directionality
const hemiLight = new THREE.HemisphereLight(
  new THREE.Color(0.02, 0.02, 0.04), // near-black sky (no blue bias)
  new THREE.Color(0.01, 0.01, 0.01), // near-black ground
  0.12
);
scene.add(hemiLight);

// Primary sunlight — warm golden-white, physically powerful
const pointLight = new THREE.PointLight(new THREE.Color(1.0, 0.96, 0.85), 5.0, 1400, 0.45);
pointLight.position.set(0, 0, 0);
pointLight.castShadow = false; // off for performance — sun glow handles the feel
scene.add(pointLight);

// Very faint interplanetary scatter — barely perceptible, cool-tinted
const fillLight = new THREE.PointLight(new THREE.Color(0.10, 0.18, 0.55), 0.12, 600, 1);
fillLight.position.set(90, 50, -170);
scene.add(fillLight);

// Galaxy background — cinematic deep-space canvas texture
function createGalaxyTexture() {
  ...
  // ── Base: near-pure-black with subtlest indigo undertone ──
  ctx.fillStyle = '#000002';
  ctx.fillRect(0, 0, w, h);

  // Barely-there background gradient: deep left/right edge tint
  const bgTint = ctx.createLinearGradient(0, 0, w, h);
  bgTint.addColorStop(0,   'rgba(8,4,28,0.22)');   // dark indigo
  bgTint.addColorStop(0.45,'rgba(0,0,0,0)');
  bgTint.addColorStop(0.55,'rgba(0,0,0,0)');
  bgTint.addColorStop(1,   'rgba(10,4,24,0.18)');  // subtle violet
  ctx.fillStyle = bgTint;
  ctx.fillRect(0, 0, w, h);

  // ── Nebula clouds ── all indigo/violet — max opacity 0.16 (no blue dome) ──
  const nebulas = [
    // Primary upper-left cluster — deep indigo-violet
    { x: 0.10, y: 0.20, r: 0.38, c: '24,12,78',  a: 0.16 },
    { x: 0.06, y: 0.12, r: 0.24, c: '30,14,88',  a: 0.12 },
    { x: 0.18, y: 0.34, r: 0.20, c: '18,8,65',   a: 0.13 },
    { x: 0.12, y: 0.24, r: 0.16, c: '48,10,82',  a: 0.09 }, // purple inner
    // Secondary upper-right — dark violet
    { x: 0.88, y: 0.18, r: 0.28, c: '20,10,74',  a: 0.14 },
    { x: 0.84, y: 0.30, r: 0.18, c: '25,12,70',  a: 0.10 },
    { x: 0.93, y: 0.10, r: 0.16, c: '32,14,85',  a: 0.08 },
    // Galactic core hint — center, dark warm purple
    { x: 0.50, y: 0.24, r: 0.30, c: '38,10,62',  a: 0.08 },
    { x: 0.47, y: 0.32, r: 0.20, c: '28,8,55',   a: 0.06 },
    // Lower nebula wisps — very faint depth layers
    { x: 0.28, y: 0.60, r: 0.22, c: '14,6,50',   a: 0.06 },
    { x: 0.70, y: 0.64, r: 0.20, c: '18,8,55',   a: 0.05 },
    { x: 0.50, y: 0.82, r: 0.25, c: '10,4,40',   a: 0.05 },
    // Far edge wisps — wrap continuity
    { x: 0.02, y: 0.42, r: 0.16, c: '20,8,62',   a: 0.07 },
    { x: 0.98, y: 0.48, r: 0.16, c: '20,8,62',   a: 0.07 },
    // Mid-scene faint cloud
    { x: 0.38, y: 0.45, r: 0.18, c: '12,5,44',   a: 0.04 },
    { x: 0.62, y: 0.50, r: 0.16, c: '15,6,48',   a: 0.04 },
  ];
  nebulas.forEach(({ x, y, r, c, a }) => {
    const grd = ctx.createRadialGradient(x*w, y*h, 0, x*w, y*h, r*w);
    grd.addColorStop(0,    `rgba(${c},${a})`);
    grd.addColorStop(0.30, `rgba(${c},${(a*0.55).toFixed(3)})`);
    grd.addColorStop(0.65, `rgba(${c},${(a*0.18).toFixed(3)})`);
    grd.addColorStop(1,    'rgba(0,0,0,0)');
    ctx.fillStyle = grd;
    ctx.fillRect(0, 0, w, h);
  });

  // ── Milky Way band — main + secondary offset for depth ──
  const mwA = ctx.createLinearGradient(0, h*0.26, w, h*0.74);
  mwA.addColorStop(0,    'rgba(0,0,0,0)');
  mwA.addColorStop(0.36, 'rgba(22,18,65,0.07)');
  mwA.addColorStop(0.50, 'rgba(30,22,75,0.11)');
  mwA.addColorStop(0.64, 'rgba(22,18,65,0.07)');
  mwA.addColorStop(1,    'rgba(0,0,0,0)');
  ctx.fillStyle = mwA;
  ctx.fillRect(0, 0, w, h);

  const mwB = ctx.createLinearGradient(0, h*0.18, w, h*0.82);
  mwB.addColorStop(0,    'rgba(0,0,0,0)');
  mwB.addColorStop(0.42, 'rgba(16,12,52,0.04)');
  mwB.addColorStop(0.58, 'rgba(16,12,52,0.04)');
  mwB.addColorStop(1,    'rgba(0,0,0,0)');
  ctx.fillStyle = mwB;
  ctx.fillRect(0, 0, w, h);

  // ── Galaxy arm streak (Milky Way core diagonal) ──
  // Dense micro-star concentration along a diagonal — looks like a distant galaxy arm
  const mwAngle = 0.88; // radians
  const mwCx = w * 0.52, mwCy = h * 0.46;
  const mwLen = w * 0.60, mwW = h * 0.055;
  for (let i = 0; i < 320; i++) {
    const t   = (rng(1) - 0.5) * mwLen;
    const falloff = 1 - Math.abs(t / (mwLen * 0.5));
    const pOff = (rng(1) - 0.5) * mwW * falloff;
    const px = mwCx + Math.cos(mwAngle) * t - Math.sin(mwAngle) * pOff;
    const py = mwCy + Math.sin(mwAngle) * t + Math.cos(mwAngle) * pOff;
    if (px < 0 || px > w || py < 0 || py > h) continue;
    const sz = rng(0.25) + 0.05;
    const br = (rng(0.14) + 0.05).toFixed(2);
    ctx.beginPath();
    ctx.arc(px, py, sz, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(195,210,255,${br})`;
    ctx.fill();
  }

  // Dense star cluster regions (upper-left and upper-right, matching nebulas)
  [[0.11*w, 0.21*h], [0.87*w, 0.20*h]].forEach(([cx, cy]) => {
    for (let i = 0; i < 60; i++) {
      const dx = (rng(1)-0.5)*w*0.08, dy = (rng(1)-0.5)*h*0.10;
      const px = cx + dx, py = cy + dy;
      if (px < 0 || px > w || py < 0 || py > h) continue;
      const br = (rng(0.14) + 0.04).toFixed(2);
      ctx.beginPath();
      ctx.arc(px, py, rng(0.22)+0.04, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(200,215,255,${br})`;
      ctx.fill();
    }
  });

  // ── Deep background stars (layer 1) — 1100 ultra-tiny ──
  for (let i = 0; i < 1100; i++) {
    const x = rng(w), y = rng(h);
    const r = rng(0.24) + 0.05;
    const bright = (rng(0.22) + 0.08).toFixed(2); // max 0.30
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(212,220,255,${bright})`;
    ctx.fill();
  }

  // ── Medium stars (layer 2) — 420 ──
  for (let i = 0; i < 420; i++) {
    const x = rng(w), y = rng(h);
    const r = rng(0.42) + 0.12;
    const bright = (rng(0.20) + 0.14).toFixed(2); // max 0.34
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(220,226,255,${bright})`;
    ctx.fill();
  }

  // ── Colored accent stars (layer 3) — 90 varied spectral types ──
  for (let i = 0; i < 90; i++) {
    const x = rng(w), y = rng(h);
    const r = rng(0.55) + 0.18;
    const bright = (rng(0.28) + 0.20).toFixed(2); // max 0.48
    const t = Math.random();
    let col;
    if (t < 0.28)      col = `rgba(148,192,255,${bright})`; // O/B-type blue giant
    else if (t < 0.52) col = `rgba(255,244,200,${bright})`; // G-type warm yellow
    else if (t < 0.70) col = `rgba(255,208,148,${bright})`; // K-type orange
    else if (t < 0.84) col = `rgba(200,210,255,${bright})`; // A-type blue-white
    else               col = `rgba(255,230,210,${bright})`; // M-type warm white
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fillStyle = col;
    ctx.fill();
  ...
  // ── Rare bright accent (layer 4) — 22 slightly larger stars ──
  // max 0.50 sRGB → linear 0.21 → well below bloom 0.85
  for (let i = 0; i < 22; i++) {
    const x = rng(w), y = rng(h);
    const r = rng(0.70) + 0.28;
    const bright = (rng(0.20) + 0.30).toFixed(2); // 0.30–0.50
    const t = Math.random();
    const col = t < 0.5
      ? `rgba(160,200,255,${bright})`
      : `rgba(255,248,220,${bright})`;
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fillStyle = col;
    ctx.fill();
...
// Background sphere — full-sky galaxy dome, never visible as a shape
const galaxyTex = createGalaxyTexture();
const galaxyGeo = new THREE.SphereGeometry(500, 64, 32);
...
// Sun — extremely bright HDR core; toneMapped:false → raw values → bloom picks them up
const sunMaterial = new THREE.MeshBasicMaterial({
  map: loader.load("/textures/sun.jpg"),
  toneMapped: false,
  color: new THREE.Color(1.7, 1.45, 1.05) // warm golden-white HDR; bright enough to bloom, texture stays visible
});
const sun = new THREE.Mesh(new THREE.SphereGeometry(5, 64, 64), sunMaterial);
scene.add(sun);

// Sun corona — SPRITE billboards, NOT SphereGeometry.
// Sprites always face the camera → no visible sphere-edge circular ring artifacts.
// The radial gradient texture creates smooth, natural glow falloff.
function _buildCorona(size, stops) {
  const c = document.createElement('canvas');
  c.width = c.height = size;
  const ctx = c.getContext('2d');
  const h = size / 2;
  const g = ctx.createRadialGradient(h, h, 0, h, h, h);
  stops.forEach(([t, rgba]) => g.addColorStop(t, rgba));
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, size, size);
  return new THREE.CanvasTexture(c);
}

// Tight inner corona: bright warm falloff just outside the sun sphere
const _coronaTex1 = _buildCorona(512, [
  [0.00, 'rgba(255,245,200,0.00)'],
  [0.18, 'rgba(255,225,120,0.55)'],  // peak just outside sun radius
  [0.35, 'rgba(255,195,60,0.25)'],
  [0.55, 'rgba(255,160,30,0.08)'],
  [0.78, 'rgba(255,130,10,0.02)'],
  [1.00, 'rgba(0,0,0,0)'],
]);

// Wide soft halo: golden amber wash, very low opacity
const _coronaTex2 = _buildCorona(512, [
  [0.00, 'rgba(255,210,90,0.00)'],
  [0.10, 'rgba(255,200,70,0.18)'],
  [0.28, 'rgba(255,170,42,0.08)'],
  [0.50, 'rgba(255,145,22,0.025)'],
  [0.75, 'rgba(255,120,10,0.005)'],
  [1.00, 'rgba(0,0,0,0)'],
]);

// Far light scatter: barely-visible ambient warmth
const _coronaTex3 = _buildCorona(256, [
  [0.00, 'rgba(255,190,50,0.00)'],
  [0.08, 'rgba(255,185,45,0.06)'],
  [0.28, 'rgba(255,160,28,0.02)'],
  [0.55, 'rgba(255,140,15,0.005)'],
  [1.00, 'rgba(0,0,0,0)'],
]);

function _makeCoronaSprite(tex, worldScale, initOp) {
  ...
  sp.scale.setScalar(worldScale);
  return sp;
}

// Scale in world units: sun radius = 5. Multiples:
//   _sunGlowA: 16 → r=8  = 1.6× sun (tight halo)
//   _sunGlowB: 38 → r=19 = 3.8× sun (wide soft halo)
//   _sunGlowC: 72 → r=36 = 7.2× sun (faint far scatter)
const _sunGlowA = _makeCoronaSprite(_coronaTex1, 16, 0.92);
const _sunGlowB = _makeCoronaSprite(_coronaTex2, 38, 0.90);
const _sunGlowC = _makeCoronaSprite(_coronaTex3, 72, 0.82);
sun.add(_sunGlowA);
sun.add(_sunGlowB);
sun.add(_sunGlowC);

// sunCoronas: animation loop pulses their material.opacity
const sunCoronas = [
  { mesh: _sunGlowA, baseOp: 0.88, pulseAmp: 0.12 },
  { mesh: _sunGlowB, baseOp: 0.84, pulseAmp: 0.10 },
  { mesh: _sunGlowC, baseOp: 0.76, pulseAmp: 0.12 },
];
...
const celestialBodies = [
  ...
  },
  {
    ...
    speed: 0.000012,
    initialAngle: 1.2,
    color: new THREE.Color(0.69, 0.93, 0.93),
    roughness: 0.85,
    metalness: 0.0,
    type: "planet",
  ...
  },
  {
    ...
    speed: 0.0000061,
    initialAngle: 6.1,
    color: new THREE.Color(0.12, 0.25, 0.77),
    roughness: 0.85,
    metalness: 0.0,
    type: "planet",
...
celestialBodies.forEach((body) => {
  ...
  if (body.texture) {
    ...
    material = new THREE.MeshStandardMaterial({
      ...
      emissive: new THREE.Color(0.014, 0.012, 0.020), // dark-side never fully black
    ...
    material = new THREE.MeshStandardMaterial({
      ...
      metalness: body.metalness || 0.05,
      roughness: body.roughness || 1,
      emissive: body.color ? body.color.clone().multiplyScalar(0.035) : new THREE.Color(0.01, 0.01, 0.02),
  ...
  if (body.type === 'dwarf') {
    orbitColor = new THREE.Color(0.85, 0.65, 0.0);
    glowIntensity = 0.10;
    baseOpacity = 0.055;
  } else if (body.type === 'asteroid') {
    orbitColor = new THREE.Color(0.65, 0.35, 0.15);
    glowIntensity = 0.08;
    baseOpacity = 0.045;
  } else if (body.type === 'tno') {
    orbitColor = new THREE.Color(0.45, 0.18, 0.55);
    glowIntensity = 0.12;
    baseOpacity = 0.065;
  } else {
    if (body.dist < 20) {
      orbitColor = new THREE.Color(0.35, 0.55, 0.80);
      glowIntensity = 0.06;
      baseOpacity = 0.042;
    } else if (body.dist < 35) {
      orbitColor = new THREE.Color(0.55, 0.45, 0.80);
      glowIntensity = 0.09;
      baseOpacity = 0.055;
    } else {
      orbitColor = new THREE.Color(0.75, 0.35, 0.48);
      glowIntensity = 0.12;
      baseOpacity = 0.065;
    }
  }
...
bloomPass.strength = 0.38;  // restrained: bright but not planet-washing
bloomPass.radius = 0.42;   // tight corona, doesn't flood the scene
bloomPass.threshold = 0.85; // sun fires bloom; sprite coronas and stars stay below
...
// Round star sprite — 16×16 radial gradient eliminates square GL_POINTS artifact
function createStarSprite() {
  ...
  g.addColorStop(0.55, 'rgba(210,220,255,0.30)');
  g.addColorStop(1,    'rgba(0,0,0,0)');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, 16, 16);
  return new THREE.CanvasTexture(c);
}
const _starSprite = createStarSprite();

// Helper — generate a sphere-distributed point cloud
function _spherePoints(count, minR, maxR) {
...
// Helper — build a Points object and add to scene
function _makeStar(count, minR, maxR, sz, col, op) {
  const geo = _spherePoints(count, minR, maxR);
...
// 6-layer star field with per-group twinkling metadata
// All opacities ≤ 0.48, all sizes ≤ 0.40 → safely below bloom threshold 0.85
function createStarLayers() {
  const layers = [
    // [count, minR, maxR, size, color, baseOp, twinkleAmp, twinkleSpeed]
    [90, 130, 380, 0.18, new THREE.Color(0.72, 0.80, 0.98), 0.28, 0.00, 0.00],  // far tiny cold — static
    [75, 110, 300, 0.20, new THREE.Color(0.90, 0.90, 0.94), 0.26, 0.03, 0.12],  // far tiny warm — barely twinkle
    [60, 70,  140, 0.24, new THREE.Color(0.76, 0.84, 1.00), 0.34, 0.08, 0.22],  // mid cool — slow twinkle
    [48, 55,  110, 0.26, new THREE.Color(0.96, 0.92, 0.82), 0.32, 0.08, 0.28],  // mid warm — slow twinkle
    [28, 38,  75,  0.30, new THREE.Color(1.00, 0.97, 0.82), 0.38, 0.18, 0.40],  // near warm — medium twinkle
    [12, 45,  95,  0.38, new THREE.Color(0.88, 0.95, 1.00), 0.44, 0.26, 0.55],  // rare bright — visible twinkle
  ];

  const groups = layers.map(([cnt, mn, mx, sz, col, op, amp, spd]) => {
    const pts = _makeStar(cnt, mn, mx, sz, col, op);
    return { pts, baseOp: op, amp, speed: spd, phase: Math.random() * Math.PI * 2 };
  });

  return groups;
}
const _starGroups = createStarLayers();
// distantStars ← the first group (main layer) kept for eclipse tour compatibility
const distantStars = _starGroups[0].pts;

// Space dust — 650 sub-pixel warm-white micro-particles inside the solar system
// Size 0.038 at distance 20–60 → ≈ sub-pixel. Visible only when light catches them.
function createSpaceDust() {
  ...
  geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
  const mat = new THREE.PointsMaterial({
    color: new THREE.Color(0.96, 0.93, 0.85),
    size: 0.038,
    transparent: true,
    opacity: 0.075,
    sizeAttenuation: true,
...
const _spaceDust = createSpaceDust();

// Distant galaxy clusters — 2 dense clusters at far distance, anchored near nebula regions
// Size 0.18 at r≈350 → sub-pixel. Appear as faint star smudges / galaxy patches.
function createGalaxyClusters() {
  const clusterDefs = [
    { cx: -205, cy:  88, cz: -325, count: 55, spread: 16, col: new THREE.Color(0.70, 0.76, 0.95), op: 0.17 },
    { cx:  245, cy:  72, cz: -285, count: 42, spread: 13, col: new THREE.Color(0.68, 0.74, 0.92), op: 0.15 },
  ];
  return clusterDefs.map(({ cx, cy, cz, count, spread, col, op }) => {
    const pos = new Float32Array(count * 3);
    for (let i = 0; i < count; i++) {
      // Gaussian-like distribution: sum of two uniforms → bell-shaped cluster
      const rx = (Math.random() + Math.random() - 1) * spread;
      const ry = (Math.random() + Math.random() - 1) * spread * 0.45;
      const rz = (Math.random() + Math.random() - 1) * spread;
      pos[i*3]   = cx + rx;
      pos[i*3+1] = cy + ry;
      pos[i*3+2] = cz + rz;
    }
...
const _galaxyClusters = createGalaxyClusters();
...
function animate() {
  ...
  p.mesh.rotation.y += 0.01 * realTimeMultiplier * cinematicFactor;
    ...
    // Multi-layer star field — all groups rotate slightly, each at different speed
    const _ct = Date.now() * 0.001;
    _starGroups.forEach((g, i) => {
      // Slow differential rotation — outer layers barely move
      g.pts.rotation.y += (0.000045 + i * 0.000012) * realTimeMultiplier;
      // Twinkling — random independent oscillation per group
      if (g.amp > 0) {
        g.pts.material.opacity = g.baseOp + g.amp * (Math.sin(_ct * g.speed + g.phase) * 0.5 + 0.5);
      }
    });

    // Space dust — very slow independent drift
    _spaceDust.rotation.y += 0.000025 * realTimeMultiplier;
    _spaceDust.rotation.x += 0.000010 * realTimeMultiplier;

    // Sun corona pulsing — each layer breathes at a different slow frequency
    sunCoronas.forEach((c, i) => {
      const pulse = Math.sin(_ct * (0.30 + i * 0.15) + i * 1.4) * 0.5 + 0.5;
      c.mesh.material.opacity = c.baseOp + c.pulseAmp * pulse;
    });

    // Planetary micro-float — sub-unit Y-oscillation for organic depth
    planetMeshes.forEach((p, i) => {
      p.mesh.position.y = Math.sin(_ct * 0.20 + i * 0.95) * 0.08;
    });
  }
  ...
  if (bloomPass && !isBloomManual) {
    bloomPass.strength = 0.32 + (1 - normalizedDistance) * 0.28; // 0.32–0.60 (close = stronger)
    bloomPass.radius = 0.35 + (1 - normalizedDistance) * 0.18;
  } else if (bloomPass && isBloomManual) {
    bloomPass.strength = manualBloomStrength;
    bloomPass.radius = 0.35 + (1 - normalizedDistance) * 0.12;
  }
...
if (bloomControl && bloomValue) {
  isBloomManual = true;
  manualBloomStrength = 0.38;
  bloomPass.strength = 0.38;
  bloomControl.value = 0.38;
  bloomValue.textContent = '0.38';
