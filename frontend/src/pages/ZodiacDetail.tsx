...
// ─── Circle Gauge ─────────────────────────────────────────────────────────────
function CircleGauge({value,label,color="#C9A84C"}:{value:number;label:string;color?:string}) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
      ...
      <span style={{fontSize:16,color:"rgba(201,168,76,0.85)",textTransform:"uppercase",letterSpacing:"0.08em",textAlign:"center",fontWeight:700}}>{label}</span>
    </div>
  );
}
  ...
  return (
    <div style={{perspective:700,cursor:"pointer"}} onClick={()=>setFlipped(f=>!f)}>
      <div style={{width:200,height:310,position:"relative",transformStyle:"preserve-3d",transition:"transform 0.7s ease",transform:flipped?"rotateY(180deg)":"rotateY(0)"}}>
        <div style={{position:"absolute",inset:0,backfaceVisibility:"hidden",borderRadius:18,background:"linear-gradient(135deg,#1a0533,#2d1b4e)",border:"2px solid #C9A84C",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12}}>
          <div style={{fontSize:60}}>🔮</div>
          <div style={{color:"rgba(201,168,76,0.7)",fontSize:15,letterSpacing:"0.15em",fontWeight:600}}>TAP TO REVEAL</div>
        </div>
        <div style={{position:"absolute",inset:0,backfaceVisibility:"hidden",borderRadius:18,background:"linear-gradient(160deg,#1a0533,#0d0d1a)",border:"2px solid #C9A84C",transform:"rotateY(180deg)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,padding:28,textAlign:"center"}}>
          <div style={{fontSize:44}}>✨</div>
          <div style={{color:"#C9A84C",fontWeight:800,fontSize:22,letterSpacing:"0.08em"}}>{card}</div>
...
function FortuneWheel({rewards,sign}:{rewards:string[];sign:string}) {
  ...
  function drawWheel(){
    ...
    rewards.forEach((_,i)=>{
      ...
      ctx.fillStyle="#C9A84C";ctx.font="bold 12px sans-serif";ctx.textAlign="right";
      ctx.fillText("✨",r-20,5);ctx.restore();
    });
    ctx.beginPath();ctx.arc(cx,cy,18,0,2*Math.PI);ctx.fillStyle="#C9A84C";ctx.fill();
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:24}}>
      <div style={{position:"relative"}}>
        <canvas ref={canvasRef} width={300} height={300} style={{borderRadius:"50%",border:"3px solid #C9A84C",boxShadow:"0 0 40px rgba(201,168,76,0.3)",transform:`rotate(${rotation}deg)`,transition:spinning?"transform 3s cubic-bezier(0.17,0.67,0.12,0.99)":"none"}}/>
        <div style={{position:"absolute",top:-18,left:"50%",transform:"translateX(-50%)",color:"#C9A84C",fontSize:28}}>▼</div>
      </div>
      {result&&<div style={{background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.4)",borderRadius:14,padding:"18px 28px",textAlign:"center",maxWidth:320}}><div style={{color:"#C9A84C",fontSize:17,fontWeight:700,marginBottom:6}}>Your Fortune</div><div style={{color:"#e8e0f0",fontSize:16}}>{result}</div></div>}
      <button onClick={spin} disabled={spinning||spunToday} style={{padding:"16px 44px",background:spunToday?"rgba(255,255,255,0.05)":"linear-gradient(135deg,#C9A84C,#a07830)",border:"none",borderRadius:30,color:spunToday?"rgba(255,255,255,0.3)":"#000",fontWeight:800,fontSize:18,cursor:spunToday?"not-allowed":"pointer",letterSpacing:"0.05em"}}>
...
export default function ZodiacDetail() {
  ...
  const ttl:React.CSSProperties = {color:"#C9A84C",fontWeight:800,fontSize:16,letterSpacing:"0.2em",textTransform:"uppercase",display:"flex",alignItems:"center",gap:12,marginBottom:28};
  const crd:React.CSSProperties = {background:"rgba(255,255,255,0.035)",border:"1px solid rgba(201,168,76,0.14)",borderRadius:16,padding:"24px"};

  if (!zodiac || !daily) return (
    <Layout>
      <div style={{minHeight:"60vh",display:"flex",alignItems:"center",justifyContent:"center",color:"#C9A84C",fontSize:24}}>Loading cosmic data...</div>
    </Layout>
  );
  ...
  return (
    <Layout>
      <style>{`
        @keyframes pulse-glow{0%,100%{box-shadow:0 0 24px rgba(201,168,76,0.35)}50%{box-shadow:0 0 60px rgba(201,168,76,0.75)}}
        @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-12px)}}
        @keyframes shimmer{0%{background-position:200% center}100%{background-position:-200% center}}
        @keyframes fade-in{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
        @keyframes word-glow{0%,100%{text-shadow:0 0 14px rgba(201,168,76,0.4)}50%{text-shadow:0 0 36px rgba(201,168,76,0.95),0 0 70px rgba(201,168,76,0.45)}}
        ...
        .hcard:hover{transform:translateY(-4px);border-color:rgba(201,168,76,0.4)!important}
        .bfill{transition:width 1.4s cubic-bezier(0.4,0,0.2,1)}
        .back-link:hover{color:#C9A84C!important}
        .signav:hover{color:#C9A84C!important}
      `}</style>
      ...
      <div style={{width:"100%",maxWidth:1280,margin:"0 auto",padding:"0 40px 80px",color:"#e8e0f0",boxSizing:"border-box"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 0 32px",borderBottom:"1px solid rgba(201,168,76,0.1)",marginBottom:36}}>
          <Link to="/zodiac" className="back-link"
            style={{color:"rgba(201,168,76,0.8)",textDecoration:"none",fontSize:16,letterSpacing:"0.12em",fontWeight:700,display:"flex",alignItems:"center",gap:8,transition:"color 0.2s"}}>
            ← ALL SIGNS
          </Link>
        ...
        <div style={{display:"grid",gridTemplateColumns:"420px 1fr",gap:48,marginBottom:36,alignItems:"center"}}>
          {/* Zodiac image box */}
          <div style={{background:"rgba(255,255,255,0.025)",borderRadius:24,padding:44,display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid rgba(201,168,76,0.14)",animation:"float 6s ease-in-out infinite",minHeight:340}}>
            {zodiacImg && !imgError ? (
              <img src={zodiacImg} alt={zodiac.name} onError={()=>setImgError(true)}
                style={{width:"100%",maxWidth:300,maxHeight:300,objectFit:"contain",filter:"drop-shadow(0 0 28px rgba(201,168,76,0.5))"}}/>
            ) : (
              <div style={{fontSize:140,filter:"drop-shadow(0 0 28px rgba(201,168,76,0.55))",lineHeight:1}}>{zodiac.symbol}</div>
            )}
          </div>
          {/* Sign details */}
          <div>
            <div style={{fontSize:14,color:"rgba(201,168,76,0.55)",letterSpacing:"0.25em",marginBottom:12,fontWeight:600}}>DAILY COSMIC READING</div>
            ...
            <div style={{color:"rgba(201,168,76,0.9)",fontSize:20,marginBottom:22,fontWeight:600}}>{zodiac.dates}</div>
            <div style={{display:"flex",gap:12,marginBottom:22,flexWrap:"wrap"}}>
              <span style={{background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.35)",borderRadius:24,padding:"7px 20px",fontSize:16,color:"#C9A84C",fontWeight:600}}>Element: {zodiac.element}</span>
              <span style={{background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.35)",borderRadius:24,padding:"7px 20px",fontSize:16,color:"#C9A84C",fontWeight:600}}>Ruler: {zodiac.ruling}</span>
        ...
        <div className="zd-sec" style={{...sec,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:20}}>
          <div style={{display:"flex",alignItems:"center",gap:20}}>
            ...
            <div>
              <div style={{color:"#C9A84C",fontWeight:800,fontSize:26}}>{streak}-Day Cosmic Streak</div>
              <div style={{color:"rgba(255,255,255,0.45)",fontSize:16,marginTop:4}}>Return daily to grow your streak</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:16,background:"rgba(201,168,76,0.07)",border:"1px solid rgba(201,168,76,0.24)",borderRadius:16,padding:"16px 28px"}}>
            <span style={{fontSize:36}}>{badge.icon}</span>
            <div>
              <div style={{color:"#C9A84C",fontWeight:800,fontSize:20}}>{badge.name}</div>
        ...
        <div className="zd-sec" style={{...sec,textAlign:"center"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:24}}>
            <div style={{position:"relative",width:200,height:200}}>
              <div style={{position:"absolute",inset:0,borderRadius:"50%",background:`conic-gradient(#C9A84C ${energyCount*3.6}deg, rgba(255,255,255,0.05) 0)`,animation:"pulse-glow 3s ease-in-out infinite",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <div style={{width:162,height:162,borderRadius:"50%",background:"#0d0d1a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
                  <div style={{fontSize:46,fontWeight:900,color:"#C9A84C",lineHeight:1}}>{energyCount}%</div>
                ...
                <div key={key} className="hcard" style={crd}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <span style={{fontSize:15,fontWeight:700,letterSpacing:"0.08em",color:"#C9A84C",textTransform:"uppercase"}}>{HORO_LABELS[key]}</span>
                    </div>
                    <span style={{fontSize:20,fontWeight:800,color:h.score>=80?"#4ade80":h.score>=65?"#C9A84C":"#f87171"}}>{h.score}%</span>
                  </div>
                  <div style={{background:"rgba(255,255,255,0.06)",borderRadius:6,height:6,overflow:"hidden",marginBottom:14}}>
                    <div className="bfill" style={{height:"100%",width:`${h.score}%`,background:"linear-gradient(90deg,#C9A84C,#f0d080)",borderRadius:6}}/>
                  </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:16}}>
            {[
              ...
              <div key={item.label} style={{...crd,textAlign:"center"}}>
                ...
                <div style={{fontSize:12,color:"rgba(201,168,76,0.65)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>{item.label}</div>
        ...
        <div className="zd-sec" style={sec}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:20,justifyItems:"center"}}>
            ...
            <CircleGauge value={daily.opportunities.career} label="Career" color="#C9A84C"/>
        ...
        <div className="zd-sec" style={{...sec,textAlign:"center"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:48,flexWrap:"wrap",padding:"10px 0"}}>
            {daily.powerWords.map((w,i)=>(
              <div key={i} className="pword" style={{fontSize:36,fontWeight:900,color:"#C9A84C",letterSpacing:"0.1em",textTransform:"uppercase",animationDelay:`${i*0.7}s`}}>{w}</div>
            ))}
          </div>
        </div>
        ...
        <div className="zd-sec" style={{...sec,textAlign:"center"}}>
          ...
          <div style={{fontSize:34,fontWeight:900,color:"#C9A84C",letterSpacing:"0.1em",marginBottom:10}}>{daily.mood}</div>
        ...
        <div className="zd-sec" style={sec}>
          ...
          <div style={{background:"rgba(201,168,76,0.05)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:18,padding:"32px",textAlign:"center"}}>
            ...
            <div style={{fontSize:15,color:"rgba(201,168,76,0.7)",letterSpacing:"0.12em",fontWeight:600}}>REWARD: +10 Cosmic Energy</div>
          </div>
        </div>
        ...
        <div className="zd-sec" style={{...sec,textAlign:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:40,flexWrap:"wrap"}}>
            <div style={{textAlign:"center"}}>
              ...
              <div style={{color:"#C9A84C",fontWeight:700,fontSize:20,marginTop:10,textTransform:"capitalize"}}>{zodiac.name}</div>
            </div>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:44,fontWeight:900,color:"#C9A84C"}}>{daily.compatibility.score}%</div>
            ...
            <div style={{textAlign:"center"}}>
              ...
              <div style={{color:"#C9A84C",fontWeight:700,fontSize:20,marginTop:10}}>{daily.compatibility.sign}</div>
            </div>
          </div>
        ...
        <div className="zd-sec" style={sec}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:24,flexWrap:"wrap"}}>
            <div style={{width:80,height:80,borderRadius:"50%",background:"linear-gradient(135deg,#C9A84C,#8B5E2A)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,flexShrink:0}}>⭐</div>
            <div>
              <div style={{color:"#C9A84C",fontWeight:800,fontSize:24,marginBottom:8}}>{daily.celebrity.name}</div>
        ...
        <div className="zd-sec" style={{...sec,textAlign:"center"}}>
          ...
          <div style={{fontSize:22,fontStyle:"italic",lineHeight:1.85,maxWidth:620,margin:"0 auto",background:"linear-gradient(135deg,#C9A84C,#f0d080,#C9A84C)",backgroundSize:"200% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",animation:"shimmer 4s linear infinite"}}>
            "{daily.manifestation}"
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:28}}>
          <div className="zd-sec" style={{...sec,marginBottom:0}}>
            ...
            <div style={{marginBottom:18}}>
              <div style={{fontSize:13,color:"rgba(201,168,76,0.65)",letterSpacing:"0.18em",marginBottom:12,fontWeight:600}}>CORE TRAITS</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
                {zodiac.traits.map(t=><span key={t} style={{background:"rgba(201,168,76,0.09)",border:"1px solid rgba(201,168,76,0.24)",borderRadius:22,padding:"6px 16px",fontSize:15,color:"#C9A84C",fontWeight:600}}>{t}</span>)}
              </div>
            </div>
            <div style={{marginBottom:18}}>
              <div style={{fontSize:13,color:"rgba(201,168,76,0.65)",letterSpacing:"0.18em",marginBottom:12,fontWeight:600}}>STRENGTHS</div>
              {zodiac.strengths.map(s=><div key={s} style={{color:"rgba(232,224,240,0.7)",fontSize:17,marginBottom:6}}>☆ {s}</div>)}
            </div>
            <div>
              <div style={{fontSize:13,color:"rgba(201,168,76,0.65)",letterSpacing:"0.18em",marginBottom:12,fontWeight:600}}>CHALLENGES</div>
          ...
          <div className="zd-sec" style={{...sec,marginBottom:0}}>
            ...
            <div style={{fontSize:13,color:"rgba(201,168,76,0.65)",letterSpacing:"0.18em",marginBottom:12,fontWeight:600}}>BEST MATCHES</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
              {zodiac.compatibility.map(m=>(
                <Link key={m} to={`/zodiac/${m.toLowerCase()}`}
                  style={{background:"rgba(201,168,76,0.09)",border:"1px solid rgba(201,168,76,0.24)",borderRadius:22,padding:"7px 16px",fontSize:15,color:"#C9A84C",textDecoration:"none",display:"flex",alignItems:"center",gap:8,fontWeight:600}}>
        ...
        {/* ── TOMORROW PREVIEW ── */}
        <div className="zd-sec" style={{...sec,textAlign:"center",border:"1px solid rgba(201,168,76,0.25)",background:"linear-gradient(135deg,rgba(201,168,76,0.05),rgba(0,0,0,0))"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
            ...
            <span style={{fontSize:22,fontWeight:800,color:"#C9A84C",fontVariantNumeric:"tabular-nums"}}>{countdown}</span>
          </div>
        </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:16,flexWrap:"wrap",gap:16}}>
          <Link to={`/zodiac/${prevSign}`} className="signav"
            style={{display:"flex",alignItems:"center",gap:10,color:"rgba(201,168,76,0.7)",textDecoration:"none",fontSize:18,fontWeight:700,transition:"color 0.2s"}}>
            ← {SIGN_SYMBOLS[prevSign]} {prevSign.charAt(0).toUpperCase()+prevSign.slice(1)}
          </Link>
          <Link to="/compatibility"
            style={{background:"linear-gradient(135deg,#C9A84C,#a07830)",color:"#000",fontWeight:800,fontSize:17,letterSpacing:"0.1em",padding:"16px 36px",borderRadius:32,textDecoration:"none"}}>
            CHECK COMPATIBILITY ♡
          </Link>
